 ...
 */
package com.baidu.bikenavi_demo.search;
...
import com.baidu.bikenavi_demo.overlay.util.BikingRouteOverlay;
import com.baidu.bikenavi_demo.overlay.util.DrivingRouteOverlay;
import com.baidu.bikenavi_demo.overlay.util.MassTransitRouteOverlay;
import com.baidu.bikenavi_demo.overlay.util.OverlayManager;
import com.baidu.bikenavi_demo.overlay.util.TransitRouteOverlay;
import com.baidu.bikenavi_demo.overlay.util.WalkingRouteOverlay;
 ...
 * 同时展示如何进行节点浏览并弹出泡泡
 */
public class RoutePlanDemo extends MyActivity implements BaiduMap.OnMapClickListener,
    ...
    public void searchButtonProcess(View v) {
        ...
        PlanNode stNode = PlanNode.withCityNameAndPlaceName("贺州", startNodeStr);
        PlanNode enNode = PlanNode.withCityNameAndPlaceName("贺州", endNodeStr);

        PlanNode stNode2 = PlanNode.withCityNameAndPlaceName("贺州", "贺州市正菱大酒店");
        PlanNode enNode2 = PlanNode.withCityNameAndPlaceName("贺州", "广西贺州市职业学院");

        PlanNode stNode3 = PlanNode.withCityNameAndPlaceName("贺州", "广西贺州市职业学院");
        PlanNode enNode3 = PlanNode.withCityNameAndPlaceName("贺州", "贺州广济医院");
        ...
        if (v.getId() == R.id.mass) {
            ...
            mSearch.walkingSearch((new WalkingRoutePlanOption())
                    .from(stNode3).to(enNode3));
            mSearch.drivingSearch((new DrivingRoutePlanOption()).from(enNode3).to(enNode));
            Log.d(TAG, "searchButtonProcess: "+stNode2);
            nowSearchType = 3;
    ...
    public void nodeClick(View v) {
        ...
        if (nowSearchType != 0 && nowSearchType != -1) {
            ...
            Log.i(TAG, "nodeClickroute.getAllStep : "+route.getAllStep());
              ...
              //  Log.i(TAG, "nodeLocation: "+nodeLocation+"\n"+"nodeTitle : "+nodeTitle);
    ...
    public void onGetMassTransitRouteResult(MassTransitRouteResult result) {
        ...
        if (result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
            // 起终点模糊，获取建议列表
            result.getSuggestAddrInfo();
            Toast.makeText(this, result.getSuggestAddrInfo()+"", Toast.LENGTH_SHORT).show();
            return;
        }
        if (result.error == SearchResult.ERRORNO.NO_ERROR) {
